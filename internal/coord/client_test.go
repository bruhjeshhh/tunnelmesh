...
func TestClient_Register(t *testing.T) {
	...
	// Register
	resp, err := client.Register("mynode", "SHA256:abc123", []string{"1.2.3.4"}, []string{"192.168.1.1"}, 2222)
	require.NoError(t, err)
...
func TestClient_ListPeers(t *testing.T) {
	...
	// Register a peer
	_, err = client.Register("node1", "SHA256:key1", nil, nil, 2222)
	require.NoError(t, err)
...
func TestClient_Heartbeat(t *testing.T) {
	...
	// Register first
	_, err = client.Register("mynode", "SHA256:key", nil, nil, 2222)
	require.NoError(t, err)
...
func TestClient_GetDNSRecords(t *testing.T) {
	...
	_, err = client.Register("node1", "SHA256:key1", nil, nil, 2222)
	require.NoError(t, err)
	_, err = client.Register("node2", "SHA256:key2", nil, nil, 2222)
	require.NoError(t, err)
